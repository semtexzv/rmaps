//! Automatically generated rust module for 'tile2.proto' file
...
impl<'a> MessageRead<'a> for Feature {
    fn from_reader(r: &mut BytesReader, bytes: &'a [u8]) -> Result<Self> {
        let mut msg = Feature {
            type_pb: tile2::GeomType::UNKNOWN,
            ..Self::default()
        };
        while !r.is_eof() {
            match r.next_tag(bytes) {
...
impl MessageWrite for Feature {
    fn get_size(&self) -> usize {
        ...
        + if self.type_pb == tile2::GeomType::UNKNOWN { 0 } else { 1 + sizeof_varint(*(&self.type_pb) as u64) }
    ...
    fn write_message<W: Write>(&self, w: &mut Writer<W>) -> Result<()> {
        ...
        if self.type_pb != tile2::GeomType::UNKNOWN { w.write_with_tag(24, |w| w.write_enum(*&self.type_pb as i32))?; }
...
pub struct Layer<'a> {
    pub version: u32,
    pub name: Cow<'a, str>,
    pub features: Vec<tile2::Feature>,
    pub keys: Vec<Cow<'a, str>>,
    pub values: Vec<tile2::Value<'a>>,
    pub extent: u32,
}
    ...
    fn from_reader(r: &mut BytesReader, bytes: &'a [u8]) -> Result<Self> {
        ...
        while !r.is_eof() {
            match r.next_tag(bytes) {
                ...
                Ok(18) => msg.features.push(r.read_message::<tile2::Feature>(bytes)?),
                Ok(26) => msg.keys.push(r.read_string(bytes).map(Cow::Borrowed)?),
                Ok(34) => msg.values.push(r.read_message::<tile2::Value>(bytes)?),
...
impl<'a> MessageRead<'a> for Tile<'a> {
    fn from_reader(r: &mut BytesReader, bytes: &'a [u8]) -> Result<Self> {
        ...
        while !r.is_eof() {
            match r.next_tag(bytes) {
                Ok(26) => msg.layers.push(r.read_message::<tile2::Layer>(bytes)?),
                Ok(t) => { r.read_unknown(bytes, t)?; }
                Err(e) => return Err(e),
            }
        }
