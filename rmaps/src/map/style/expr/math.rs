...
fn eval_num(e: &BaseExpr, ctx: &EvaluationContext) -> StdResult<f64, ExprEvalError> {
    let e = e.eval(ctx)?;
...
impl Expr for Math {
    ...
    fn eval(&self, ctx: &EvaluationContext) -> ExprResult {
        ...
        return Ok(match (&self.0, &self.1[..]) {
            ...
            _ => {
                return Err(ExprEvalError::custom(format!("Mismatchet math operation and arguments : {:?} used on {:?}", self.0, self.1)));
            }
        }.into());
    }
}